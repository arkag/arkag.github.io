Title: Github Pages and Pelican
Date: 2020-07-31 12:34
Modified: 
Category: DevOps
Tags: pelican, travis-ci, github-pages
Slug: github-pages-and-pelican
Authors: Alex Kagno
Summary: How to use Travis CI to publish a Pelican site automatically to Github Pages

A better write-up coming soon. I just want to test my CI/CD pipeline right now.

### References

1. [Primary documentation](https://humberto.io/blog/publishing-at-github-pages-with-pelican-and-travis-ci/)
2. [Pelican Alchemy](https://github.com/nairobilug/pelican-alchemy)
3. [Pelican documenation](https://docs.getpelican.com/en/stable/)